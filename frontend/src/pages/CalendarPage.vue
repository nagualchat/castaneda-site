  ...
  <div id="calendar">
    ...
    <div class="calendar-nav-wrapper">
      <div class="calendar-nav-group">
        <div v-if="view==='year'" v-on:click="prevYear()" class="level-item invert-link is-borderless"><b-icon icon="chevron-left"></b-icon></div> 
        <div v-if="view==='year'" class="is-size-5">{{ year }}</div>
        <div v-if="view==='year'" v-on:click="nextYear()" class="level-item invert-link is-borderless"><b-icon icon="chevron-right"></b-icon></div>

        <div v-if="view==='month'" v-on:click="prevMonth()" class="level-item invert-link is-borderless"><b-icon icon="chevron-left"></b-icon></div> 
        <div v-if="view==='month'" class="is-size-5">{{ dateFormatted }}</div>
    ...
    <div class="calendar-content-wrapper">
      <div class="calendar-view">
        <div v-for="month in items" v-if="month.display" class="calendar-month" v-bind:key="month.month">
          <div class="calendar-month-name is-size-7">{{ month.name }}</div>
          <div class="calendar-days-names is-size-7">
          ...
          <div v-for="week in month.weeks" class="calendar-week">
            <div v-for="day in week.days" v-on:click="selectDay(new Date(day.timestamp), day.relateToMonth)" class="calendar-day-wrapper" :class="{ relate: day.relateToMonth, today: day.today, selected: view==='month' && day.selected, cursorPointer: view==='month' && !day.relateToMonth }">
              <div class="calendar-day is-size-7">{{ day.date }}</div>
  ...
  cycledMarks: [
    {
      ...
      pattern: [
        ...
        false,
        true
      ], //паттерн длиной 18 дней
      description:
        "Каждые 18 дней на Землю падает волна энергии. Следующий раз будет третьего августа. Вы сможете почувствовать это." +
...
export default {
  name: "CalendarPage",
    metaInfo: {
      title: '18 дневный цикл',
      meta: [
        { vmid: 'description', name: 'description', content: 'Рассчёт дат 18-дневного цикла, о котором рассказывал Карлос Кастанеда в одном из интервью.' }
      ]
    },
  data() {
    return {
...
function isDateInPattern(baseDate, date, n) {
  //базовая дата, проверяемая дата, число дней через которое идет повторение
...
function getItems(fromDate, toDate) {
  ...
  while (fromDate <= toDate) {
    ...
    if (0 in weeksFromNextMonthBuffer) {
      weeksFromCurrentMonthBuffer = invertRelationToMonth(
        weeksFromCurrentMonthBuffer
      );
      weeksFromNextMonthBuffer[0].days = weeksFromCurrentMonthBuffer.concat(
        weeksFromNextMonthBuffer[0].days
      );
      items[monthCount].weeks = items[monthCount].weeks.concat(
    ...
    while (items[monthCount].weeks.length < 6) {
      ...
      if (fromDate.getDay() === 1) {
        ...
        if (!relateToMonth) {
          if (!bufferTriggered) {
            weeksFromCurrentMonthBuffer = items[monthCount].weeks[
              items[monthCount].weeks.length - 1
            ].days.slice(
              0,
              items[monthCount].weeks[items[monthCount].weeks.length - 1].days
                .length - 1
            );
          ...
          weeksFromNextMonthBuffer[
            weeksFromNextMonthBuffer.length - 1
          ].days.push(pushDayObject(fromDate, true));
        }
...
#calendar {
  ...
  .calendar-view {
    ...
    .selected {
      border: 1px solid $selectedDayBorderColor;
    }
